...
var brickX = (canvasWid / 2) - (brickWid / 2);
var stackSpeed = 5; //how fast to move the stack
var stackSpeedBoost = 2; //how much speed to add to stack movement each time
var currentKey;
...
function draw() {
    ...
    if (mouseX > 60 && mouseX < 280 && mouseY > 330 && mouseY < 430) {
      ...
      if (clickMade) {
        ...
        if (mouseButton === LEFT) {
          ...
          brickApearTime = 5;
          fallingSpeed = 1;
          brickErrorMargin = brickHei / 20;
          stackSpeed = 5;
          brickX = (canvasWid / 2) - (brickWid / 2);
        }
      }
    } else {
      fill(0,128,0);
  ...
  if (mode === 1) { //mode 1, the game
    ...
    //check to see if there are even any falling blocks before the rest
    ...
    if (timerValue !== 0 && timerValue % 15 === 0) {
      ...
      if (lastSpeedTime !== timerValue) {
        ...
        if (fallingSpeed < 13) {       
          ...
          //make the stack move more for each key keyPress
          stackSpeed += stackSpeedBoost;
    ...
    if (lives < 1) {
      ...
      timerValue = 0;
      lives = difficulty;
      //score = 0; //leave the score intact so it can be shown on the death screen
      stack = [
        {brickOffX: 0,
        brickOffY: 0,
        colour: '#770029'}
    ...
    noStroke();
    text("Time: " + timerValue,15,40);
    text("Score: " + score,15,70);
    //text("High Score: " + highScore,15,100);
...
//the function for keyboard interaction
function keyPressed() {
  if (true) {
...
function loadHighScore() {
  loadStrings("highScore.txt", fileLoaded);
}
